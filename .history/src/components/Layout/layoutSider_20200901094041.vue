    ...
    >
      <template v-for="item in menus">
        ...
        <el-submenu :index="item.path" v-if="item.children" :key="item.path">
          ...
          <el-menu-item-group>
            <el-menu-item :index="`/${item1.path}`" v-for="item1 in item.children" :key="item1.id" @click="addNav(item1)">
...
export default {
  ...
  methods: {
    ...
    addNav(item){
      ...
      this.authList.push({
        name:item.authName,
        path:`/${item.path}`})
      this.authList = this.authList.filter((item1,index,arr) => {
        return arr.indexOf(item1.authName) === item.authName
      })
  ...
  mounted() {
    ...
    if(localStorage.getItem(`${this.user.username}Nav`)){
      this.authList = JSON.parse(localStorage.getItem(`${this.user.username}Nav`))
