      ...
      <el-form :model="formOptions" :rules="rules" ref="form">
        <template v-for="(item,index) in formData">
          <el-form-item :label="item.label" :prop="item.prop" :key="index" v-if="!item.children">
            <!-- 动态渲染组件 -->
            <component :is="`el-${item.type}`" v-model="formOptions[item.prop]"  v-bind="item.attrs" v-if="item.type ==='cascader'"></component>
            <component :is="`el-${item.type}`" v-model="formOptions[item.prop]"  v-bind="item.attrs" v-else></component>
          </el-form-item>
