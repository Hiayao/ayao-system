<template>
  <div class="sort_All">
    <div class="box">
      <el-button type="primary" class="btn" @click="Visible = true">添加分类</el-button>
      <ug-form
...
export default {
  ...
  methods: {
    ...sortActions(["getCategories", "Addcategories"]),
    ...
    ok(val) {
      val.formRef.validate((valid) => {
        if (valid) {
          ...
          console.log(val);
          val.formRef.resetFields();
          if (val.options.cat_pid[0] === "") {
            this.cat_pid = "0";
            this.cat_level = "0";
          }
          console.log(val.options.cat_name);
          this.$store.dispatch('sort/Addcategories',{
