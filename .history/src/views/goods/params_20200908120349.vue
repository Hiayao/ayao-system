<template>
  <div class="box">
    ...
    <ug-form :Visible.sync="Visible" @cancel="cancel" @ok="ok" :formData="formData" @input="input" :title='title'></ug-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...AttributesActions(["getAttributes", "Addattributes"]),
    ...
    ok(val) {
      val.formRef.validate((valid) => {
        if (valid) {
          ...
          console.log(val);
          val.formRef.resetFields();
          this.Addattributes({ id:this.id, attr_name:this.val, attr_sel:this.sel, attr_vals:this.attr_vals,sel:this.sel });
        } else {
